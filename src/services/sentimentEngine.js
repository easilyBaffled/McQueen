...
const POSITIVE_KEYWORDS = {
  high: [
    'touchdown', 'td', 'winning', 'career-high', 'record-breaking', 'mvp',
    'game-winning', 'clutch', 'dominant', 'historic', 'pro bowl', 'all-pro',
    'triple crown', 'franchise record', 'league-leading'
  ],
  medium: [
    'scores', 'leads', 'dominates', 'surges', 'breakout', 'explosive',
    'outstanding', 'impressive', 'excellent', 'strong', 'big game',
    'key play', 'crucial', 'rallies', 'comeback', 'hot streak'
  ],
  low: [
    'solid', 'consistent', 'returns', 'practice', 'cleared', 'healthy',
    'ready', 'full participation', 'no limitations', 'good shape',
    'confident', 'optimistic', 'progress', 'improving'
  ]
};
...
const NEGATIVE_KEYWORDS = {
  high: [
    'injury', 'torn', 'out for season', 'arrested', 'suspended', 'acl',
    'season-ending', 'surgery', 'fracture', 'broken', 'serious',
    'indefinitely', 'career-threatening', 'released', 'cut'
  ],
  medium: [
    'questionable', 'limited', 'struggling', 'benched', 'interception',
    'fumble', 'turnover', 'sacked', 'ineffective', 'concerning',
    'doubtful', 'setback', 'aggravated', 'flare-up', 'missed practice'
  ],
  low: [
    'minor', 'rest', 'maintenance', 'dnp', 'veteran day off',
    'precautionary', 'day-to-day', 'slight', 'manageable',
    'load management', 'not serious'
  ]
};

// Special context keywords that modify sentiment
const CONTEXT_MODIFIERS = {
  negation: ['not', 'no', 'won\'t', 'didn\'t', 'isn\'t', 'wasn\'t', 'unlikely', 'denied'],
...
const POSITION_KEYWORDS = {
  QB: {
    positive: ['passing yards', 'completions', 'passer rating', 'qbr', 'clean pocket'],
  ...
  WR: {
    positive: ['targets', 'receptions', 'receiving yards', 'separation', 'contested catch'],
...
const FANTASY_KEYWORDS = {
  positive: [
    'target share', 'snap count increase', 'red zone', 'usage', 'workload',
    'featured', 'starter', 'wr1', 'rb1', 'every down back', 'alpha'
  ],
  negative: [
    'committee', 'timeshare', 'reduced role', 'backup', 'spelled',
    'pitch count', 'limited snaps', 'third down only'
  ]
};
...
export function analyzeSentiment(text, playerName = '', position = '') {
  ...
  if (position && POSITION_KEYWORDS[position]) {
    POSITION_KEYWORDS[position].positive.forEach(keyword => {
      if (lowerText.includes(keyword.toLowerCase())) {
        foundKeywords.push({ word: keyword, type: 'positive', level: 'medium' });
        positiveScore += 1.5;
      }
    });
    POSITION_KEYWORDS[position].negative.forEach(keyword => {
      if (lowerText.includes(keyword.toLowerCase())) {
        foundKeywords.push({ word: keyword, type: 'negative', level: 'medium' });
        negativeScore += 1.5;
      }
    });
  }
...
function isNegationNearKeyword(text, keyword) {
  ...
  const keywordIndex = words.findIndex(w => w.includes(keyword.toLowerCase().split(' ')[0]));
