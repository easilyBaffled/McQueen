  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <>
          <motion.div
            className={styles['glossary-backdrop']}
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            onClick={onClose}
          />
          <motion.div
            className={styles['glossary-panel']}
            role="dialog"
            aria-modal="true"
          ...
          >
            <div className={styles['glossary-header']}>
              <div className={styles['glossary-title-row']}>
                <h2 id="glossary-title" className={styles['glossary-title']}>
                  📖 Trading Terms
                </h2>
                <button
                  className={styles['glossary-close']}
                  onClick={onClose}
              ...
              <p className={styles['glossary-subtitle']}>
                New to stock trading? Here's what everything means.
              </p>
              <div className={styles['glossary-search']}>
                <svg
                  viewBox="0 0 24 24"
                  fill="currentColor"
            ...
            <div className={styles['glossary-content']}>
              {filteredTerms.map((item, index) => (
                <motion.div
                  key={item.term}
                  className={styles['glossary-item']}
                  initial={{ opacity: 0, y: 10 }}
                  animate={{ opacity: 1, y: 0 }}
                  transition={{ delay: index * 0.03 }}
                >
                  <h3 className={styles['glossary-term']}>{item.term}</h3>
                  <p className={styles['glossary-definition']}>{item.definition}</p>
                  <p className={styles['glossary-example']}>
                    <span className={styles['example-label']}>Example:</span>{' '}
                    {item.example}
                  </p>
                </motion.div>
              ))}
              ...
              {filteredTerms.length === 0 && (
                <div className={styles['glossary-empty']}>
                  <p>No terms match "{searchQuery}"</p>
                </div>
              )}
            </div>

            <div className={styles['glossary-footer']}>
              <p>💡 Tip: This is all play money! Experiment freely.</p>
              <button
                className={styles['restart-tutorial-btn']}
                onClick={() => {
...
export function useTermTooltip() {
  ...
  const shouldShowTooltip = (term: string) => !seenTerms.includes(term);
