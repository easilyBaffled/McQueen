...
export default function InfoTooltip({ term, children, inline = false }: InfoTooltipProps) {
  ...
  return (
    <span
      className={`${styles['info-tooltip-wrapper']} ${inline ? styles['inline'] : ''}`}
      onMouseEnter={() => setIsVisible(true)}
      onMouseLeave={() => setIsVisible(false)}
    >
      {children}
      <button
        className={styles['info-tooltip-trigger']}
        onClick={(e) => {
          e.preventDefault();
          e.stopPropagation();
      ...
      <AnimatePresence>
        {isVisible && (
          <motion.div
            className={styles['info-tooltip-content']}
