...
export default function Portfolio() {
  const { portfolio, getPlayer, getPlayers, getPortfolioValue, cash } = useGame();
    ...
    <div className="portfolio-page">
      ...
      <div className="portfolio-summary">
        ...
        <div className="summary-card">
          ...
          <span className="summary-value">${cash.toLocaleString('en-US', { minimumFractionDigits: 2 })}</span>
        </div>
        <div className="summary-card">
          <span className="summary-label">
            <InfoTooltip term="portfolio">Invested</InfoTooltip>
          </span>
          <span className="summary-value">${portfolioStats.value.toLocaleString('en-US', { minimumFractionDigits: 2 })}</span>
        </div>
        <div className={`summary-card ${portfolioStats.gain >= 0 ? 'positive' : 'negative'}`}>
          <span className="summary-label">
            <InfoTooltip term="gainLoss">Total Gain/Loss</InfoTooltip>
          </span>
          <span className={`summary-value ${portfolioStats.gain >= 0 ? 'text-up' : 'text-down'}`}>
            {portfolioStats.gain >= 0 ? '+' : ''}${portfolioStats.gain.toLocaleString('en-US', { minimumFractionDigits: 2 })}
            <span className="summary-percent">({portfolioStats.gain >= 0 ? '+' : ''}{portfolioStats.gainPercent.toFixed(2)}%)</span>
          </span>
        </div>
      </div>
      ...
      {holdings.length === 0 ? (
        ...
        >
          ...
          <p>Build your portfolio by investing in NFL players you believe will outperform the market.</p>
              ...
              <div className="suggested-list">
                {trendingPlayers.map((player) => (
                  ...
                  >
                    ...
                    <span className={`suggested-change ${player.changePercent >= 0 ? 'up' : 'down'}`}>
            ...
            >
              <Link to={`/player/${holding.id}`} className="holding-row">
                <div className="holding-player">
                  ...
                  <span className="player-team">{holding.team} • {holding.position}</span>
                </div>
                <span className="holding-shares">{holding.shares}</span>
                <span className="holding-cost">${holding.avgCost.toFixed(2)}</span>
                <span className={`holding-current ${holding.changePercent >= 0 ? 'text-up' : 'text-down'}`}>
                  ${holding.currentPrice.toFixed(2)}
                </span>
                <span className="holding-value">${holding.currentValue.toFixed(2)}</span>
                <span className={`holding-gain ${holding.gain >= 0 ? 'text-up' : 'text-down'}`}>
                  {holding.gain >= 0 ? '+' : ''}${holding.gain.toFixed(2)}
                  <span className="gain-percent">({holding.gain >= 0 ? '+' : ''}{holding.gainPercent.toFixed(1)}%)</span>
                </span>
              </Link>
            </motion.div>
          ))}
