            ...
            {missionRevealed ? (
              <div className="mission-results">
                <div className="results-header">
                  ...
                  <div className="results-percentile">
                    You beat <span className="percentile-value">{score?.percentile}%</span> of traders!
                  </div>
                </div>
                    ...
                    {missionPicks.risers.map((id) => {
                      ...
                      return (
                        <div key={id} className={`result-chip ${status}`}>
                          <span className="result-name">{player?.name || id}</span>
                          <span className={`result-change ${player?.changePercent >= 0 ? 'up' : 'down'}`}>
                            {player?.changePercent >= 0 ? '▲' : '▼'} {Math.abs(player?.changePercent || 0).toFixed(1)}%
                          </span>
              ...
              <>
                <div className="picks-grid">
                      ...
                      {missionPicks.risers.map((id) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <button onClick={() => clearMissionPick(id)}>×</button>
                          </motion.div>
                        );
                      })}
                      {[...Array(3 - missionPicks.risers.length)].map((_, i) => (
                        <div key={`empty-riser-${i}`} className="pick-chip empty">
                          Select a riser
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="picks-column fallers">
                    ...
                    <div className="picks-list">
                      ...
                      {[...Array(3 - missionPicks.fallers.length)].map((_, i) => (
                        <div key={`empty-faller-${i}`} className="pick-chip empty">
                          Select a faller
                        </div>
                      ))}
                    </div>
                ...
                <div className="player-selector">
                  <p className="selector-hint">Click a player to add them to your picks:</p>
                  <div className="selector-chips">
                    {players.slice(0, 12).map((player) => {
                      ...
                      return (
                        <div key={player.id} className={`selector-chip ${isPicked ? 'picked' : ''}`}>
                          <span className="selector-name">{player.name}</span>
                          <span className={`selector-change ${player.changePercent >= 0 ? 'up' : 'down'}`}>
                          ...
                          <div className="selector-actions">
                            <button
                              ...
                              disabled={missionPicks.risers.length >= 3 && !isRiser}
                            >
                              ▲
                            </button>
                            <button
                              className={`selector-btn down ${isFaller ? 'active' : ''}`}
                              onClick={() => setMissionPick(player.id, 'faller')}
                              disabled={missionPicks.fallers.length >= 3 && !isFaller}
                            >
                              ▼
                            </button>
                          </div>
                ...
                >
                  {canReveal ? 'Reveal Results!' : `Select ${6 - totalPicks} more players`}
                </button>
              </>
            )}
          </motion.div>
